...
import { FixedPoint} from '../../uniswap-lib/libraries/FixedPoint.sol';
...
contract ExampleOracleSimple {
  ...
  function getAmountOut(address tokenIn, uint amountIn) external view returns (uint amountOut) {
    ...
    if( tokenIn == token0 ) {
      return UniswapV2Library.getAmountOut( amountIn, reserve0, reserve1 );
    } else {
      if( tokenIn == token1 ) {
        return UniswapV2Library.getAmountOut( amountIn, reserve1, reserve0 );
      }
    }
  }
}
